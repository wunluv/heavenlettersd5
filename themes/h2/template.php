...
function phptemplate_forum_icon($new_posts, $num_posts = 0, $comment_mode = 0, $sticky = 0) {
  ...
  $output = '<i data-feather="' . $feather_icons[$icon] . '"></i>';
  ...
  switch ($hook) {
    ...
    case 'page':
      if (drupal_is_front_page()) {
        ...
        $vars['testimonials_view'] = views_embed_view('testimonials');
        $vars['products_view'] = views_embed_view('products');
      }
      ...
      if (arg(0) == 'heavenletters' && arg(1) == 'archive') {
        $vars['template_file'] = 'page-archive';
        $vars['heavenletters_archive_view'] = views_embed_view('heavenletters_archive');
      }
      break;
  }
...
function phptemplate_feed_icon($url, $title) {
  return '<a href="' . $url . '" class="feed-icon" title="' . $title . '"><i data-feather="rss"></i></a>';
}
